 ...
 *
 * Local copy for us-station.html, adapted 2026-09-12: identical to the
 * real ../@@RUNTIME@@/scripts/weather-data.js, except its "./weather.json",
 * "./data/history/" and "./config/panels.json" paths were rewritten to
 * "../..." (one directory up) because this page lives one folder deeper
 * than the pages that normally load the real script directly. The real
 * scripts/weather-data.js itself was not touched.
 */
...
(function (window) {
  ...
  async function load() {
    ...
    const livePath = "@@RUNTIME@@/weather.json?v=" + Date.now();
  ...
  async function loadHistory(days = 7) {
    ...
    const fetchedDays = await Promise.all(
      datesToFetch.map((iso) =>
        fetchJson("@@RUNTIME@@/data/history/" + iso + ".json").catch(() => null)
      )
    );
  ...
  async function loadPanels() {
    return fetchJson("@@RUNTIME@@/config/panels.json");
  }
